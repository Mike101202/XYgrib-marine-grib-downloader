...
# Version: 2.27 - 2026-09-20
#
# v2.27 -- critical fix + concurrency + 384h/day-based UI
#   * CRITICAL: do_download() now validates connect_timeout/max_timeout/
#     retries as positive integers before ever building the curl args,
#     falling back to safe defaults (20/300/2) if not. Root cause of
#     the reported bug: a bad/empty CONNECT_TIMEOUT reached curl
#     identically on every parallel worker at once, so all of them
#     failed with "expected a proper numerical parameter" in the same
#     instant -- and with PARALLEL=16, sixteen processes writing that
#     to the same stderr simultaneously is what produced the
#     unreadable, character-interleaved garbage in the report. curl's
#     own stderr is now suppressed in favor of the script's own single
#     clean retry message, so even a genuine (non-corrupted-config)
#     curl error can't produce that interleaving under high PARALLEL.
#   * validate_download_settings(): same defensive pattern applied to
#     PARALLEL/SLEEP/CONNECT_TIMEOUT/MAX_TIMEOUT/HYCOM_*/*_PARALLEL_MAX
#     -- resets bad values immediately in change_download() and once
#     at startup, so a corrupted config can't reach this state again.
#   * probe_all() is now skipped under --download (unattended mode)
#     unless --status was also given -- run_download() never reads its
#     result (each download function does its own live
#     server_reachable() check instead), so the up-to-~48s probe was
#     pure overhead on every cron/anacron run.
#   * [N] Days now offers up to 16 days (384h) with a 12-day option
#     too, and is framed as the recommended path; [4] Forecast (raw
#     hours/interval) is now labeled as the advanced alternative.
#     build_hours_list()/validate_forecast_settings() already supported
#     the full 384h range correctly (confirmed, not changed) -- the fix
#     here is exposing it as an actual choice instead of capping the
#     picker at 240h.
#   * GFS+Wave+RTOFS/HYCOM/SeaTemp now download the NOMADS leg
#     (atmos+wave) and the currents leg (RTOFS/HYCOM) concurrently
#     instead of sequentially -- different servers, independently
#     probed parallel limits, no rate-limit reason to serialize them.
#     Falls back to sequential under Satellite mode, since that mode's
#     whole point is minimizing simultaneous connection pressure. Each
#     branch's output is captured to its own temp log and printed in
#     full, in order, once both finish -- letting two live progress
#     displays interleave on one terminal is exactly the kind of
#     unreadable mess the timeout bug above also produced, so this
#     avoids it by construction rather than relying on luck. LAST_CYCLE/
#     LAST_RATE (set inside the backgrounded NOMADS branch) are
#     recovered via a small state file, since a subshell's variable
#     changes don't propagate back to the parent shell.
#
# v2.26 -- --status flag, added to support the GUI wrapper
#   (marine-grib-downloader-gui.py). Loads config, probes servers and
#   dependencies exactly like startup already did, then prints a
#   machine-readable summary (current config, last cycle, and the raw
#   contents of ~/.grib_downloader_status) and exits -- no download,
#   no menu. Existing --download/interactive behavior is unchanged.
#
# v2.25 -- critical fix: stale-cycle collision
#   Local atmos/wave filenames only ever encoded the cycle HOUR
#   (t12z, t18z...), never the DATE. GFS cycles recur at the same
#   four hours every day, so a file downloaded for today's 12z was
#   named identically to yesterday's 12z, or any previous day's. Since
#   do_download() treats "a valid GRIB file already at this path" as
#   "already downloaded, skip it," a leftover file from a previous
#   day's same-hour cycle was silently reused as if it were current --
#   the download would report "41/41 already present" and complete in
#   0 seconds, having fetched nothing, while looking identical to a
#   genuinely up-to-date run. This is the bug reported against v2.24.
#   Fixed by embedding the actual date into every local atmos/wave
#   filename (gfs.<YYYYMMDD>.t<CC>z... / gfswave.<YYYYMMDD>.t<CC>z...)
#   across count_local(), estimate_size(), download_gfs(),
#   download_gfswave(), combine_atmos(), and combine_waves(), so a
#   different day's cycle can never collide with today's on disk.
#   The remote NOMADS request itself is untouched -- only the local
#   on-disk filename changed; NOMADS's own file= parameter never
#   included a date to begin with.
#   Note: old-format (date-less) leftover files from before this fix
#   are now simply invisible to the new pattern -- they won't cause
#   harm, but won't be auto-cleaned either. Run [C] Clean options once
#   after upgrading if you want to clear them out manually.
#
# v2.24 -- collaborative fixes, credit to DrCalambre's xygrib-noaa.sh
# (https://github.com/DrCalambre/GFS_NOAA_NOMADS_Downloader_for_XyGrib-72h_GRIB2_Forecast):
#   * build_hours_list(): every GFS atmos/wave loop (count_local,
#     estimate_size, download_gfs, download_gfswave, combine_atmos,
#     combine_waves) now walks ONE shared forecast-hour array instead
#     of each recomputing "H+=INTERVAL" independently. NOMADS switches
#     from 3-hourly to 12-hourly steps at 240h regardless of the
#     configured INTERVAL; requesting forecast hours beyond 240h at
#     the old fixed step (e.g. f243/f246) would have 404'd on every
#     one of them. This is the exact class of bug xygrib-noaa.sh
#     v1.0.3 fixed (STEP mutated in one loop, stale in the others).
#     Deliberately NOT applied to ECMWF/ICON -- they don't share GFS's
#     240h rule, and copying it over would introduce a bug rather than
#     fix one.
#   * validate_forecast_settings(): clamps FORECAST_HOURS to GFS's
#     real 384h ceiling and snaps INTERVAL to a multiple of 3 (GFS's
#     real publishing granularity), catching bad/non-numeric input.
#     Wired into both change_forecast() and change_days().
#   * build_wave_url(): optional all_var=on&all_lev=on fallback
#     (WAVE_ALL_VAR, off by default) for when NCEP's wave variable
#     names go stale -- sidesteps guessing entirely, at the cost of
#     pulling every field the product publishes instead of just the
#     ones requested.
#   * report_combine_completeness(): combine_atmos()/combine_waves()
#     now count how many of the expected forecast hours actually made
#     it into the combined file and warn if it's incomplete, instead
#     of silently reporting success on a gapped GRIB.
#   * --download/-d: a non-interactive mode for cron/anacron (this
#     script had no unattended path before at all). Getting this safe
#     meant auditing every prompt reachable from run_download()'s call
#     graph -- found and fixed five separate blocking reads that would
#     have hung a cron job forever (HYCOM/ICON dependency checks,
#     ECMWF/ICON size-confirmation prompts, HYCOM's on-demand pip
#     install prompt).
#
# v2.22 fix:
...
UA="XyGrib_unx/1.2.6 (compatible; grib-downloader/2.27)"
...
# Used anywhere run_download() pauses for a keypress. Under --download
# (UNATTENDED=1) these must not block, or a cron/anacron run just hangs
# forever waiting for input that will never arrive.
pause_unless_unattended() {
...
# Builds the actual GFS-published forecast-hour list into the global
# array HOURS_LIST, honouring the real NOMADS step change at 240h
# (3-hourly through 240h, 12-hourly from 240h to 384h, regardless of
# the configured INTERVAL -- NOMADS simply has nothing else to serve
# there). Every GFS atmos/wave loop walks this ONE array instead of
# each recomputing "H+=INTERVAL" independently, so the step change can
# never desync between them. This is the exact class of bug
# DrCalambre's xygrib-noaa.sh v1.0.3 fixed (STEP mutated in one loop,
# stale in the others, silently skipping files past 240h).
# Not used for ECMWF/ICON -- they don't share GFS's 240h step rule.
HOURS_LIST=()
build_hours_list() {
    HOURS_LIST=()
    local h=0 step
    while [ "$h" -le "$FORECAST_HOURS" ]; do
        HOURS_LIST+=("$h")
        if [ "$h" -lt 240 ]; then
            step="$INTERVAL"
        elif [ $((INTERVAL % 12)) -eq 0 ] && [ "$INTERVAL" -ge 12 ]; then
            step="$INTERVAL"
        else
            step=12
        fi
...
# Clamps FORECAST_HOURS/INTERVAL to values NOMADS can actually serve
# (GFS 0.25 tops out at 384h, published on 3h steps -- 12h past 240h).
# Called from change_forecast()/change_days() and once at startup so
# an old config file from before this check exists doesn't slip past.
validate_forecast_settings() {
    local warned=0
    if ! [[ "$FORECAST_HOURS" =~ ^[0-9]+$ ]]; then
        echo "  (forecast length wasn't a number -- resetting to 240h)"
        FORECAST_HOURS=240; warned=1
    fi
    if ! [[ "$INTERVAL" =~ ^[0-9]+$ ]] || [ "$INTERVAL" -lt 1 ]; then
        echo "  (interval wasn't a positive number -- resetting to 3h)"
        INTERVAL=3; warned=1
    fi
    if [ "$FORECAST_HOURS" -gt 384 ]; then
        echo "  (GFS 0.25° tops out at 384h -- clamping)"
        FORECAST_HOURS=384; warned=1
    fi
    if [ $((INTERVAL % 3)) -ne 0 ]; then
        local snapped=$(( ((INTERVAL + 2) / 3) * 3 ))
        [ "$snapped" -lt 3 ] && snapped=3
        echo "  (GFS only publishes every 3h -- rounding interval ${INTERVAL}h -> ${snapped}h)"
        INTERVAL="$snapped"; warned=1
    fi
    [ "$warned" -eq 1 ] && echo
    return 0
}

# Prints a warning if a just-combined file is missing forecast hours
# instead of silently reporting success on a gapped GRIB. $1 = label,
# $2 = hours actually found, $3 = hours expected. Inspired by
# xygrib-noaa.sh's "abort past N failures" instinct, adapted to the
# combine step (where the actual gap count is already known) rather
# than the download step.
report_combine_completeness() {
    local label="$1" found="$2" total="$3"
    [ "$total" -le 0 ] && return 0
    [ "$found" -ge "$total" ] && return 0
    local missing=$((total - found))
    local pct=$((missing * 100 / total))
    echo "  ⚠ $label: $missing of $total forecast hours missing (${pct}%)."
    [ "$pct" -ge 20 ] && echo "    That's a lot -- check for download errors above before trusting this file."
}
...
count_local() {
    ...
    for H in "${HOURS_LIST[@]}"; do
        ...
        if [ "$kind" = "wave" ]; then
            if [ -n "$cycle" ]; then
                d="${cycle% *}"; c="${cycle##* }"
                found=$(ls "$(wave_dir)"/gfswave.${d}.t${c}z.global.${WAVE_RES}.f${F}.grib2 2>/dev/null | head -1)
            else
            ...
            if [ -n "$cycle" ]; then
                d="${cycle% *}"; c="${cycle##* }"
                found=$(ls "$(atm_dir)"/gfs.${d}.t${c}z.pgrb2*.${rtok}.f${F}.* 2>/dev/null | head -1)
            else
...
build_wave_url() {
    ...
    if [ "$WAVE_ALL_VAR" = "y" ]; then
        # Sidesteps guessing NCEP's current wave variable names entirely
        # (borrowed from DrCalambre's xygrib-noaa.sh) at the cost of
        # pulling every field the wave product publishes, not just the
        # ones toggled below -- bigger files, but immune to the
        # variable-name guess below going stale between grid revisions.
        URL+="&all_var=on&all_lev=on"
    else
...
estimate_size() {
    ...
    for H in "${HOURS_LIST[@]}"; do
        ...
        found=""
        if [ "$kind" = "wave" ]; then
            found=$(ls "$dir"/gfswave.${d}.t${c}z.global.${WAVE_RES}.f${F}.grib2 2>/dev/null | head -1)
        else
            found=$(ls "$dir"/gfs.${d}.t${c}z.pgrb2*.${rtok}.f${F}.* 2>/dev/null | head -1)
        fi
        if [ -s "$found" ]; then
...
do_download() {
    ...
    # Defensive: a bad/empty timeout value here reaches curl identically
    # on every parallel worker at once (same config, same moment), so
    # they all fail simultaneously with the same "expected a proper
    # numerical parameter" error -- and with PARALLEL=16, sixteen
    # processes writing that to the same stderr at once is exactly what
    # produces unreadable, character-interleaved garbage on screen.
    # Falling back to known-good values here means a corrupted config
    # value can never reach curl at all, from any call path.
    [[ "$connect_timeout" =~ ^[0-9]+$ ]] && [ "$connect_timeout" -ge 1 ] || connect_timeout=20
    [[ "$max_timeout" =~ ^[0-9]+$ ]] && [ "$max_timeout" -ge 1 ] || max_timeout=300
    ...
    while [ "$attempt" -le "$total" ]; do
        ...
        [ "$RESUME" = "y" ] && args+=(-C -)
        # curl's own error text is suppressed here on purpose: with many
        # parallel workers all hitting the same failure at once, their
        # raw stderr interleaves character-by-character into unreadable
        # garbage. The script's own message below is the one source of
        # truth for what went wrong.
        curl "${args[@]}" "$url" 2>/dev/null && rc=0 || rc=$?
...
download_gfs() {
    ...
    for H in "${HOURS_LIST[@]}"; do
        ...
        OUT="$ATM/gfs.${d}.t${c}z.pgrb2.${rtok}.f${F}.grib2"
...
download_gfswave() {
    ...
    for H in "${HOURS_LIST[@]}"; do
        ...
        OUT="$WAV/gfswave.${d}.t${c}z.global.${WAVE_RES}.f${F}.grib2"
...
download_hycom() {
    ...
    if ! python3 -c "import xarray, netCDF4" 2>/dev/null; then
        ...
        if [ "$UNATTENDED" -eq 1 ]; then
            echo "  --download: not installing packages mid-run. Run install_grib_downloader.sh first."
            return 1
        fi
...
download_ecmwf() {
    ...
    local go
    if [ "$UNATTENDED" -eq 1 ]; then
        go="y"; echo "  --download: proceeding without a prompt."
    else
...
# ============================================================
combine_atmos() {
    local cycle="${1:-}" c="" d="" rtok; rtok=$(res_token)
    if [ -n "$cycle" ]; then d="${cycle% *}"; c="${cycle##* }"; fi
    local dir; dir=$(atm_dir)
    local final="$OUTPUT_DIR/GFS_Area_10day.grib2"; rm -f "$final"
    local H F namepat f hour_found
    build_hours_list
    local total=${#HOURS_LIST[@]} found_count=0
    for H in "${HOURS_LIST[@]}"; do
        F=$(printf "%03d" "$H")
        if [ -n "$c" ]; then namepat="gfs.${d}.t${c}z.pgrb2*.${rtok}.f${F}.grib2"
        else namepat="gfs.*.t*z.pgrb2*.${rtok}.f${F}.grib2"; fi
        hour_found=0
        while IFS= read -r -d '' f; do
            [ -s "$f" ] && { cat "$f" >> "$final"; hour_found=1; }
        done < <(find "$dir" -maxdepth 1 -name "$namepat" -print0 2>/dev/null)
        [ "$hour_found" -eq 1 ] && found_count=$((found_count + 1))
    done
    if [ -s "$final" ]; then
        echo "  Atmos → $final ($(human $(size_of "$final")))"
        report_combine_completeness "Atmos" "$found_count" "$total"
    fi
}
...
combine_waves() {
    ...
    for H in "${HOURS_LIST[@]}"; do
        F=$(printf "%03d" "$H")
        if [ -n "$c" ]; then namepat="gfswave.${d}.t${c}z.global.${WAVE_RES}.f${F}.grib2"
    ...
    if [ -s "$final" ]; then
        echo "  Waves → $final ($(human $(size_of "$final")))"
        report_combine_completeness "Waves" "$found_count" "$total"
    fi
}
...
post_download_clean_prompt() {
    ...
    if [ "$UNATTENDED" -eq 1 ]; then
        ans="y"
        echo "  --download: auto-cleaning without a prompt (AUTO_CLEAN=y)."
    else
...
show_menu() {
    ...
    echo "============================================================"
    echo "  GRIB DOWNLOADER  v2.27                       [SAT mode: ${sat_label}]"
    echo "============================================================"
...
change_forecast() {
    echo
    echo "  (Advanced: raw hours/interval. Most people want [N] Days instead --"
    echo "   it offers the same range with sane presets and validates for you.)"
...
change_days() {
    echo
    echo "  How many days of forecast do you want?"
    echo "  (GFS 0.25° publishes 3-hourly through day 10, then 12-hourly"
    echo "   out to day 16 -- build_hours_list() already handles that"
    echo "   step change correctly either way.)"
    echo
    echo "  1) 1 day    2) 2 days   3) 3 days   4) 5 days"
    echo "  5) 7 days   6) 10 days  7) 12 days  8) 16 days (max, 384h)"
    echo "  9) Custom"
    local days
    case "$(ask "Choice" "6")" in
        1) days=1;; 2) days=2;; 3) days=3;; 4) days=5;;
        5) days=7;; 6) days=10;; 7) days=12;; 8) days=16;;
        9) days=$(ask "Days (1-16)" "$(( FORECAST_HOURS / 24 ))") ;;
        *) days=10;;
    esac
    [[ "$days" =~ ^[0-9]+$ ]] || days=10
    [ "$days" -gt 16 ] && days=16
    ...
    if [ "$days" -gt 10 ]; then
        echo "  Note: past 10 days GFS drops to 12-hourly steps, and forecast"
        echo "  skill that far out is low -- useful for situational awareness,"
        echo "  not for fine-grained routing decisions."
    fi
    case "$MODEL" in
        ecmwf|icon)
            echo "  Note: this sets the shared FORECAST_HOURS value, but ECMWF/ICON"
            echo "  have their own real forecast-length limits regardless of what's"
            echo "  requested here -- the size-confirmation prompt before download"
            echo "  will show you what's actually available."
            ;;
    esac
    read -r -p "  Press Enter."
}
...
change_download() {
    ...
    WAVE_ALL_VAR=$(ask "Wave: request all_var/all_lev instead of named fields (y/n) -- use if wave files ever come back too small" "$WAVE_ALL_VAR")
    validate_download_settings
    read -r -p "  Press Enter."
}

# Rejects/resets non-numeric or out-of-range values entered above
# before they ever reach a config save or a curl invocation. do_download()
# also validates independently (belt and suspenders -- a bad value
# could otherwise reach it from a hand-edited config file, not just
# this menu), but catching it here means the person sees the problem
# immediately instead of only discovering it mid-download.
validate_download_settings() {
    local warned=0
    _num() {  # $1=name (nameref-by-eval), $2=default, $3=min
        local __name="$1" __default="$2" __min="${3:-1}"
        local __val="${!__name}"
        if ! [[ "$__val" =~ ^[0-9]+$ ]] || [ "$__val" -lt "$__min" ]; then
            echo "  ($__name wasn't a valid number -- resetting to $__default)"
            eval "$__name=\$__default"
            warned=1
        fi
    }
    _num PARALLEL 4 1
    _num SLEEP 1 0
    _num CONNECT_TIMEOUT 20 1
    _num MAX_TIMEOUT 300 1
    _num HYCOM_DAYS 1 1
    [ "${HYCOM_DAYS:-1}" -gt 8 ] 2>/dev/null && { echo "  (HYCOM_DAYS capped at 8)"; HYCOM_DAYS=8; warned=1; }
    _num HYCOM_MAX_TIMEOUT 900 1
    _num HYCOM_RETRIES 4 0
    _num WAVE_PARALLEL_MAX 4 1
    _num ICON_PARALLEL_MAX 4 1
    _num ECMWF_PARALLEL_MAX 2 1
    unset -f _num
    [ "$warned" -eq 1 ] && echo
    return 0
}
...
MENU
  ...
  [4] Forecast     - advanced: raw hours/interval (see [N] Days first)
  ...
  [N] Days         - forecast length by day count, 1-16 (recommended)
...
run_download() {
    ...
    if [ "$MODEL" = "hycom" ] || [ "$MODEL" = "hycom_seatemp" ] || \
       [ "$MODEL" = "gfs_plus_wave_hycom" ] || [ "$MODEL" = "gfs_plus_wave_seatemp" ]; then
        check_cdo || { echo "  HYCOM needs cdo."; pause_unless_unattended "  Enter."; return; }
    fi
    if [ "$MODEL" = "icon" ]; then
        check_cdo && check_wgrib2 || { echo "  ICON needs cdo + wgrib2."; pause_unless_unattended "  Enter."; return; }
    fi

    # jobs -p | xargs -r kill: if a concurrent branch below is still
    # running in the background when Ctrl+C fires, the trap alone
    # doesn't touch it -- SIGINT only interrupts whatever's in the
    # foreground. Without this, a cancelled concurrent download would
    # leave its other half running orphaned, unseen, still hitting
    # NOMADS or HYCOM after the user thinks they've stopped it.
    trap 'echo; echo; echo "  Cancelled. Partial files kept. Next [D] resumes."; \
          jobs -p | xargs -r kill 2>/dev/null; trap - INT; return' INT

    case "$MODEL" in
        gfs)
            cycle=$(detect_latest_cycle) || { echo "No cycle."; trap - INT; pause_unless_unattended "  Enter."; return; }
        ...
        gfs_plus_wave)
            ...
            pre_download_check "wave" "$cycle" || { trap - INT; return; }
            download_gfswave "$cycle"; combine_waves "$cycle" ;;
        gfs_plus_wave_rtofs|gfs_plus_wave_hycom|gfs_plus_wave_seatemp)
            cycle=$(detect_latest_cycle) || { echo "No cycle."; trap - INT; pause_unless_unattended "  Enter."; return; }
            if [ "$SATELLITE" = "y" ]; then
                # Satellite mode's whole point is minimizing simultaneous
                # connection/bandwidth pressure -- don't add a second one.
                pre_download_check "atmos" "$cycle" || { trap - INT; return; }
                download_gfs "$cycle"; combine_atmos "$cycle"
                echo
                pre_download_check "wave" "$cycle" || { trap - INT; return; }
                ...
                # NOMADS (atmos+wave) and the currents source (RTOFS or
                # HYCOM) are on different servers with independently
                # probed/throttled parallel limits -- there's no rate-
                # limit reason to run one after the other. Each branch's
                # output goes to its own log (interleaving two live
                # progress bars/spinners on one terminal is illegible,
                # and was part of what made the original bug report's
                # output unreadable) and gets printed in full, in order,
                # once both finish. LAST_CYCLE/LAST_RATE are set inside
                # download_gfs() -- which runs in a backgrounded subshell
                # below, so those variable changes never reach this
                # process directly -- and are recovered via a small
                # state file instead.
                echo "  Running NOMADS (atmos+wave) and currents concurrently"
                echo "  (different servers, no reason to serialize) ..."
                local nomads_log currents_log state_file
                nomads_log=$(mktemp) || { echo "  mktemp failed."; trap - INT; return; }
                currents_log=$(mktemp) || { echo "  mktemp failed."; rm -f "$nomads_log"; trap - INT; return; }
                state_file=$(mktemp) || { echo "  mktemp failed."; rm -f "$nomads_log" "$currents_log"; trap - INT; return; }

                (
                    pre_download_check "atmos" "$cycle" || exit 1
                    download_gfs "$cycle"; combine_atmos "$cycle"
                    echo
                    pre_download_check "wave" "$cycle" || exit 1
                    download_gfswave "$cycle"; combine_waves "$cycle"
                    printf 'LAST_CYCLE=%q\nLAST_RATE=%q\n' "$LAST_CYCLE" "$LAST_RATE" > "$state_file"
                ) > "$nomads_log" 2>&1 &
                local nomads_pid=$!

                (
                    if [ "$MODEL" = "gfs_plus_wave_rtofs" ]; then
                        pre_download_check_rtofs || exit 1
                        download_rtofs
                    else
                        download_hycom
                    fi
                ...
                while kill -0 "$nomads_pid" 2>/dev/null || kill -0 "$currents_pid" 2>/dev/null; do
                    sleep 2
                done
                wait "$nomads_pid"; local nomads_rc=$?
                wait "$currents_pid"; local currents_rc=$?

                echo
                echo "  --- NOMADS (atmos + wave) ---"
                cat "$nomads_log"
                echo
                echo "  --- Currents ($([ "$MODEL" = "gfs_plus_wave_rtofs" ] && echo RTOFS || echo HYCOM)) ---"
                cat "$currents_log"
                echo

                # shellcheck disable=SC1090
                [ -s "$state_file" ] && source "$state_file"
                rm -f "$nomads_log" "$currents_log" "$state_file"

                [ "$nomads_rc" -ne 0 ] && echo "  (NOMADS branch reported an issue -- see log above)"
                [ "$currents_rc" -ne 0 ] && echo "  (Currents branch reported an issue -- see log above)"
            fi ;;
        rtofs)
    ...
    pause_unless_unattended "  Press Enter to return to menu."
}

# ============================================================
# SECTION 16 - Entry point
# ============================================================
# --download/-d: run once using the saved config and exit, no menu.
# Suitable for cron/anacron (idea taken from xygrib-noaa.sh's anacron
# docs -- this script couldn't do unattended runs at all before).
# --status: probe servers/dependencies and print machine-readable
# status, then exit -- no download, no menu. Added for the GUI
# wrapper, so it can show server/tool status without triggering a
# real download just to check whether NOMADS is reachable.
STATUS_ONLY=0
for arg in "$@"; do
    case "$arg" in
        -d|--download) UNATTENDED=1 ;;
        --status) STATUS_ONLY=1 ;;
        -h|--help)
            cat <<USAGE
Usage: $0 [OPTIONS]

  -d, --download   Run one download using the saved config
                    (~/.grib_downloader.conf) and exit -- no menu.
                    Suitable for cron/anacron unattended runs.
  --status         Probe servers/dependencies, print status, and
                    exit -- no download, no menu.
...
if [ "$UNATTENDED" -eq 1 ] && [ "$STATUS_ONLY" -eq 0 ]; then
    # run_download() never reads probe_all()'s results -- every download
    # function does its own live server_reachable() check instead, and
    # probe_all()'s output only ever feeds the interactive menu's
    # SERVERS display (or --status below). Skipping it here saves real
    # time on every cron/anacron run (up to ~6 servers x 8s each) for no
    # functional loss.
    :
else
    probe_all
...
if [ "$STATUS_ONLY" -eq 1 ]; then
    echo "CONFIG_MODEL=$MODEL"
    echo "CONFIG_AREA=${TOP_LAT}|${LEFT_LON}|${BOTTOM_LAT}|${RIGHT_LON}"
    echo "CONFIG_FORECAST=${FORECAST_HOURS}|${INTERVAL}"
    echo "CONFIG_OUTPUT_DIR=${OUTPUT_DIR}"
    if [ -n "$LAST_CYCLE" ]; then
        echo "LAST_CYCLE=${LAST_CYCLE}"
        echo "LAST_CYCLE_AGE_H=$(cycle_age_hours "$LAST_CYCLE")"
    fi
    cat "$STATUS"
    exit 0
fi
